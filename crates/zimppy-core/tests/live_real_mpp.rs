...
async fn verify_real_mpp_payment_with_memo() {
    ...
    match &result {
        Ok(r) => {
            ...
            println!(
                "  observed amount: {} zat ({} ZEC)",
                r.observed_amount_zat,
                r.observed_amount_zat as f64 / 100_000_000.0
            );
            println!("  memo matched: {}", r.memo_matched);

            assert!(
                r.outputs_decrypted > 0,
                "should decrypt at least one output"
            );
            assert!(
                r.observed_amount_zat >= 10000,
                "should have at least 10000 zat"
            );
